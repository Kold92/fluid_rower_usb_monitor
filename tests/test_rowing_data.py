...
import tempfile
import shutil
from fluid_rower_monitor.rowing_data import RowingDataPoint, RowingSession, AllSessionsStats
...
class TestAnalyzeAllSessions:
    """Test RowingSession.analyze_all_sessions method."""
    ...
    def test_analyze_all_sessions_empty(self):
        """Test analyze_all_sessions with no sessions."""
        result = RowingSession.analyze_all_sessions(self.temp_dir)
        assert result is None

    def test_analyze_all_sessions_single_session(self):
        """Test analyze_all_sessions with one session."""
        session = RowingSession(data_dir=self.temp_dir)
        point = RowingDataPoint(
        ...
        assert isinstance(result, AllSessionsStats)
        assert result.total_sessions == 1
        assert result.total_strokes == 1
        assert result.total_distance_m == 10.0
        assert result.avg_watts_all_time == 130.0
        assert result.max_watts_all_time == 130.0

    def test_analyze_all_sessions_multiple_sessions(self):
        """Test analyze_all_sessions with multiple sessions."""
        # Session 1
        ...
        for i in range(2):
            ...
            session1.add_point(point)
        session1.save()

        # Session 2 - different start time to ensure different filenames
        session2 = RowingSession(session_start=datetime(2026, 1, 24, 11, 0), data_dir=self.temp_dir)
        for i in range(3):
            point = RowingDataPoint(
                stroke_duration_secs=2.0,
                stroke_distance_m=10.0 + i,
                time_500m_secs=135 + i,
                strokes_per_min=24,
                power_watts=150 + i * 5,
        ...
        assert result.total_distance_m == 9.0 + 10.0 + 10.0 + 11.0 + 12.0

    def test_analyze_all_sessions_returns_dataclass(self):
        """Test that analyze_all_sessions returns AllSessionsStats dataclass."""
        session = RowingSession(data_dir=self.temp_dir)
        point = RowingDataPoint(
            stroke_duration_secs=2.0,
            stroke_distance_m=10.0,
